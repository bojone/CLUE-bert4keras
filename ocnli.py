...
# 加载数据集
train_data = load_data(data_path + 'ocnli/train.50k.json')
valid_data = load_data(data_path + 'ocnli/dev.json')
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if val_acc > self.best_val_acc:
            ...
            model.save_weights('weights/ocnli.weights')
        print(
...
if __name__ == '__main__':
    ...
    model.load_weights('weights/ocnli.weights')
    test_predict(
        in_file=data_path + 'ocnli/test.json',
        out_file='results/ocnli_50k_predict.json'
    )
