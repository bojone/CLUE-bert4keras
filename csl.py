...
def load_data(filename):
    ...
    with open(filename) as f:
        for i, l in enumerate(f):
            l = json.loads(l)
            text1, text2 = '；'.join(l['keyword']), l['abst']
            label = l.get('label', 0)
...
# 加载数据集
train_data = load_data(data_path + 'csl/train.json')
valid_data = load_data(data_path + 'csl/dev.json')
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if val_acc > self.best_val_acc:
            ...
            model.save_weights('weights/csl.weights')
        print(
...
if __name__ == '__main__':
    ...
    model.load_weights('weights/csl.weights')
    test_predict(
        in_file=data_path + 'csl/test.json',
        out_file='results/csl_predict.json'
    )
